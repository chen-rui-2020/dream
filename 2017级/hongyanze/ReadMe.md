...
- [x] 学习如何使用内联框架小部件将bilibili视频、百度地图和其他媒体[嵌入原型页面](https://docs.axure.com/axure-rp/reference/inline-frames
  需要注意的地方：
  1.在链接里面加入的是嵌入代码。不要直接把网址copy进去。那样显示的是整个网页。
  2.地图的显示。百度找到的方法，先创建HTML文件把代码加进去，然后申请密钥，填写到对应的代码位置。但是需要注意IP白名单的填写。不然页面是显示不出来的。
- [x] 淘宝模糊搜索框的制作。
  使用原件：中继器，搜索框。发现中继器真的是个很好用的东东。
  还有函数[[Item.Column0.indexOf(变量名)]]的用法。模糊查询必须要掌握的
